-- 0043_active_tables.sql — الطاولة المغلقة لا تُقبل منها طلبات
--
-- المحل صار يعمل في الطابق الأرضي وحده وطاولات العلوي عُطّلت. لكن ملصق QR قديم
-- على طاولة علوية ما زال يفتح المنيو ويُدخل طلباً لطاولة لا أحد عندها — فيجلس
-- زبون ينتظر طلباً لن يصل.
--
-- ورسالة الخطأ رمز إنجليزي لا نصّ عربي: بوستغرس يعامل % في الرسالة كعلامة
-- استبدال، والترجمة مكانها التطبيق حيث تُعرض على الزبون.

drop view if exists public.active_tables;
create view public.active_tables as
  select name, kind, floor from public.cafe_tables where active;
grant select on public.active_tables to anon, authenticated;
...
begin
  ...
  -- طاولة مغلقة أو لا وجود لها: تُرفض بدل أن يجلس زبون ينتظر طلباً لن يصل.
  -- الحارس هنا لا في الصفحة: الصفحة لافتة، والدالة هي الباب — ورابط يُكتب
  -- باليد أو تبويب قديم يتجاوز أي فحص في الواجهة.
  -- والكاشير مستثنى: يكتب رقم الطاولة بيده ويرى المحل أمامه.
  if v_table is not null and p_channel <> 'cashier' then
    select t.active into v_ok from cafe_tables t where t.name = v_table;
    if v_ok is null then
      raise exception 'table_unknown';
    elsif not v_ok then
