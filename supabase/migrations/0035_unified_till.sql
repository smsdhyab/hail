-- 0035_unified_till.sql — صندوق واحد يقبض للقسمين
--
-- صار المحل كاشيراً واحداً في مكان واحد. عمود «مَن قبض»
-- (collected_by_station_id) كان يُملأ بالقسم الذي فُتح صندوقه، وليس هناك الآن
-- قسم واحد يُنسب إليه القبض.
--
-- فحين لا يُمرَّر قسم (صندوق «الكل»)، يُنسب كل طلب إلى **قسمه هو** بدل تركه
-- فارغاً: الدرج واحد، لكن نصيب الكافيه من نقوده نصيبه ونصيب المعجنات نصيبها،
-- وهذا ما يبقي مطابقة الصندوق ممكنة لكل قسم على حدة.
...
begin
  ...
  update orders o set
    ...
    status = 'paid',
    paid_at = now(),
    -- بلا قسم محدّد (صندوق «الكل») ← كل طلب يُنسب لقسمه
