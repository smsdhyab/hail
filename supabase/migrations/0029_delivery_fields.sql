-- 0029_delivery_fields.sql — حقول التوصيل + تمريرها في place_order
--
-- منفصل عن 0028 لأن قيمة enum المضافة هناك لا تُستعمل في معاملتها نفسها.
--
-- العنوان والموقع ووقت التوصيل تُخزَّن على الطلب لا على الزبون: العنوان يتغيّر
-- بين طلب وآخر (بيت، دوام، بيت أهل)، وتخزينه على بطاقة الولاء يجعل كل طلب
-- يدهس عنوان سابقه ويُوصَّل الطلب القديم إلى العنوان الجديد.

alter table public.orders add column if not exists address    text;
alter table public.orders add column if not exists geo        text;   -- رابط خريطة
alter table public.orders add column if not exists deliver_at text;   -- «الآن» أو وقت

grant select (address, geo, deliver_at) on public.orders to authenticated;

-- ── place_order يقبل بيانات التوصيل ─────────────────────────────────────────
drop function if exists public.place_order(public.order_channel, jsonb, uuid, text, text, jsonb);
...
declare
  ...
  -- capped like the note: these arrive from an anonymous public page
...
begin
  ...
  -- a delivery order has no table; ignore anything sent in that field
